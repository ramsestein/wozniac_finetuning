...
# Relative to this script (wozniac/training_bsc_bio/)
MODEL_NAME_OR_PATH = os.path.join(SCRIPT_DIR, "..", "models", "bsc-bio-ehr-es")
TRAIN_FILE = os.path.join(SCRIPT_DIR, "train.conll")
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "..", "models_results", "bsc-bio-finetuned")
...
class CoNLLDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # We assume tokens in file match tokenizer vocabulary exactly
        input_ids = self.tokenizer.convert_tokens_to_ids(tokens)
        
        # Convert labels to IDs
        label_ids = [self.label_to_id.get(l, 0) for l in labels] # Default to O if unknown
        
        # Add special tokens (CLS and SEP for RoBERTa)
        # <s> input_ids </s>
        # CLS label is -100, SEP label is -100
        ...
        # Truncate if necessary (rare given 128 max length and short sentences)
...
def main():
    ...
    training_args = TrainingArguments(
        ...
        eval_strategy="no",         # No val set for simplicity
